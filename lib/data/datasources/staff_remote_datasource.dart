...
import 'package:pos_kita/data/models/responses/staff_response_model.dart';
...
class StaffRemoteDatasource {
  ...
  StaffRemoteDatasource({AuthLocalDatasource? authLocalDatasource})
  ...
  Future<Either<String, String>> addStaff(StaffRequestModel data) async {
    try {
      ...
      return Left(response.message ?? 'Gagal menambahkan staff.');
  ...
  Future<Either<String, String>> updateStaff(
    ...
    try {
      ...
      return Left(response.message ?? 'Gagal mengubah staff.');
    } on ApiException catch (e) {
      return Left(e.message);
    } catch (_) {
      return const Left('Terjadi kesalahan tidak terduga.');
  ...
  Future<Either<String, StaffResponseModel>> getStaffs() async {
    ...
    return _wrapRequest(
      () async => _client.get<StaffResponseModel>(
        '/api/get-staff/$businessId',
        headers: await _authorizedHeaders(),
        fromJson: (json) =>
            StaffResponseModel.fromMap(json as Map<String, dynamic>),
      ),
      fallbackMessage: 'Gagal mengambil data staff.',
    );
  }
