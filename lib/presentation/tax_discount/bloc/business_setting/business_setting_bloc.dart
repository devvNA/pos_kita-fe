...
class BusinessSettingBloc
  ...
  BusinessSettingBloc(this.businessSettingRemoteDatasource)
    : super(_Initial()) {
    ...
    on<_AddBusinessSetting>((event, emit) async {
      emit(_Loading());
      final result = await businessSettingRemoteDatasource.addBusinessSetting(
        event.data,
      );
      result.fold((l) => emit(_Error(l)), (r) => add(_GetBusinessSetting()));
    });
    ...
    on<_DeleteBusinessSetting>((event, emit) async {
      emit(_Loading());
      final result = await businessSettingRemoteDatasource
          .deleteBusinessSetting(event.id);
      result.fold((l) => emit(_Error(l)), (r) => add(_GetBusinessSetting()));
    });
  }
}
