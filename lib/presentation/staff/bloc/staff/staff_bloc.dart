...
class StaffBloc extends Bloc<StaffEvent, StaffState> {
  ...
  StaffBloc(this.staffRemoteDatasource) : super(_Initial()) {
    ...
    on<_AddStaff>((event, emit) async {
      ...
      response.fold((l) => emit(_Error(l)), (r) => add(_GetStaffs()));
    });

    on<_EditStaff>((event, emit) async {
      emit(_Loading());
      final response = await staffRemoteDatasource.updateStaff(
        event.data,
        event.id,
      );
      response.fold((l) => emit(_Error(l)), (r) => add(_GetStaffs()));
