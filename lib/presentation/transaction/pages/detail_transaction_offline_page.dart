...
class DetailTransactionOfflinePage extends StatelessWidget {
  final TransactionModel transaction;
  const DetailTransactionOfflinePage({super.key, required this.transaction});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Detail Transaksi Offline'),
        leading: IconButton(
  ...
  Widget _buildHeaderInfo() {
    final isSynced = transaction.isSync == 1;
    return Container(
      ...
      child: Column(
        children: [
          ...
          _buildInfoRow(
            'Waktu',
            transaction.createdAt!.toLocal().toFormattedDateTime(),
          ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          _buildSummaryRow(
            'Subtotal',
            transaction.subTotal!.currencyFormatRpV3,
          ),
          ...
          if (transaction.discount != null && transaction.discount != '0') ...[
            ...
            _buildSummaryRow(
              'Diskon',
              '- ${transaction.discount!.currencyFormatRpV3}',
            ),
          ],
      ...
      children: [
        ...
        if (isStatus)
          Container(
            ...
            decoration: BoxDecoration(
              color: isSynced ? AppColors.success100 : AppColors.warning100,
              borderRadius: BorderRadius.circular(AppRadius.full),
            ),
            child: Text(
              value,
              style: AppTypography.labelSmall.copyWith(
                color: isSynced ? AppColors.success700 : AppColors.warning700,
                fontWeight: FontWeight.w800,
              ),
            ),
          )
  ...
  void _printReceipt(BuildContext context) async {
    final printValue = await CwbPrint.instance.printOrderV2(
      transaction.items!
          .map(
            (e) => ProductQuantity(product: e.product!, quantity: e.quantity!),
          )
          .toList(),
