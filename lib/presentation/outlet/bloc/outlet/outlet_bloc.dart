...
class OutletBloc extends Bloc<OutletEvent, OutletState> {
  ...
  OutletBloc(this.outletRemoteDatasource) : super(_Initial()) {
    ...
    on<_AddOutlet>((event, emit) async {
      ...
      response.fold((l) => emit(_Error(l)), (r) => add(_GetOutlets()));
    });

    on<_EditOutlet>((event, emit) async {
      emit(_Loading());
      final response = await outletRemoteDatasource.updateOutlet(
        event.data,
        event.id,
      );
      response.fold((l) => emit(_Error(l)), (r) => add(_GetOutlets()));
